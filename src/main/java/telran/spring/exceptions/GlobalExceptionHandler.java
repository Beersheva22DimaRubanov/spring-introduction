...
public class GlobalExceptionHandler {
	...
	ResponseEntity<String> methodArgumentHandler(MethodArgumentNotValidException e) {
		String errorMessage = e.getAllErrors().stream()
				.map(er -> er.getCodes()[0] + ": " + er.getDefaultMessage()).collect(Collectors.joining(";"));
		log.error(errorMessage);
		return new ResponseEntity<>(errorMessage, HttpStatus.BAD_REQUEST);
	}
	...
	ResponseEntity<String> constraintViolationHandler(ConstraintViolationException e){
		String errorMessage = e.getConstraintViolations().stream().map(er -> er.getPropertyPath() + ":" + er.getMessage())
